...
def home(request):
    ...
    if email_address is None:
        ...
        posts = Post.objects.filter(User_id__in=friends_ids)
            
        return render(request, 'facebook/home.html', {'user': user,'posts':posts})
...
def create_post(request):
    ...
    if request.method=='POST':
        user=user
        info_about_photo=request.POST.get('info_about_photo')
        media_file=request.FILES.get('media_file')
        new_post=Post.objects.create(User_id=user,info_about_photo=info_about_photo,media_file=media_file)
        new_post.save()
        return redirect('profile')  
    return render(request, 'facebook/post.html',{'user':user})
...
def view_all_my_posts(request):
    ...
    return render(request,'facebook/profile.html',{'user':user,'posts':posts})
...
def like(request, id):
    ...
    liked = Like.objects.filter(from_user_id=user.id, post_id=post.id).count()

    if not liked:
        Like.objects.create(from_user_id=user, post_id=post.id)
        current_likes += 1
    else:
        Like.objects.filter(from_user_id=user, post_id=post.id).delete()
        current_likes -= 1
